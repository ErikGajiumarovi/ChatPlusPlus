...
actual class FirebaseClient {
    ...
    private suspend fun getUserFromFirebaseUser(firebaseUser: GoogleFirebaseUser): User {
        ...
        return if (userDoc.exists()) {
            User(
                id = firebaseUser.uid,
                email = firebaseUser.email ?: "",
                displayName = userDoc.getString("displayName") ?: firebaseUser.displayName
    ...
    actual fun observeMessages(chatId: String): Flow<List<Message>> = callbackFlow {
    ...
    // Chat Methods
    actual suspend fun createChat(participantEmails: List<String>, name: String?): String {
        val chat = Chat(
    ...
    actual fun observeUserChats(userEmail: String): Flow<List<Chat>> = callbackFlow {
